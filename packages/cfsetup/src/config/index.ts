...
export function readConfig<CommandArgs>(
  ...
  const { config, diagnostics } = normalizeAndValidateConfig(rawConfig, configPath, args);
 ...
 */
export function findWranglerToml(referencePath: string = process.cwd(), preferJson = false): string | undefined {
  if (preferJson) {
...
export function printBindings(bindings: CfWorkerInit['bindings']) {
  ...
  if (durable_objects !== undefined && durable_objects.bindings.length > 0) {
    output.push({
      ...
      entries: durable_objects.bindings.map(({ name, class_name, script_name, environment }) => {
  ...
  if (send_email !== undefined && send_email.length > 0) {
    output.push({
      type: 'Send Email',
      entries: send_email.map(({ name, destination_address, allowed_destination_addresses }) => {
        return {
          key: name,
  ...
  if (d1_databases !== undefined && d1_databases.length > 0) {
    output.push({
      type: 'D1 Databases',
      entries: d1_databases.map(({ binding, database_name, database_id, preview_database_id }) => {
  ...
  const message = [
    ...
    ...output
      .map(bindingGroup => {
        return [`- ${bindingGroup.type}:`, bindingGroup.entries.map(({ key, value }) => `  - ${key}: ${value}`)];
      })
      .flat(2)
  ].join('\n');
