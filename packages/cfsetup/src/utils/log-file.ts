...
function getDebugFilepath() {
  ...
  const date = new Date().toISOString().replaceAll(':', '-').replace('.', '_').replace('T', '_').replace('Z', '');

  const filepath = dir.endsWith('.log')
    ? dir // allow the user to provide an exact filepath
    : path.join(dir, `cfsetup-${date}.log`);
 ...
 */
export async function appendToDebugLogFile(messageLevel: LoggerLevel, message: string) {
  const entry = `
  ...
  if (!hasLoggedLocation) {
    ...
    onExit(() => {
      ...
      if (hasSeenErrorMessage) {
        ...
        // use *console*.warn here so not to have included the *very* visible bright-yellow [WARNING] indicator
