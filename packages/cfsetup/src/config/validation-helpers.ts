 ...
 * The `fieldPath` is a dot separated property path, e.g. `"build.upload.format"`.
 */
export function experimental<T extends object>(diagnostics: Diagnostics, config: T, fieldPath: DeepKeyOf<T>): void {
  const result = unwindPropertyPath(config, fieldPath);
  if (result !== undefined && result.field in result.container) {
...
export function inheritable<K extends keyof Environment>(
  ...
  return (rawEnv[field] as Environment[K]) ?? transformFn(topLevelEnv?.[field]) ?? defaultValue;
}
...
export function inheritableInLegacyEnvironments<K extends keyof Environment>(
  ...
  return topLevelEnv === undefined || isLegacyEnv === true
    ? inheritable(diagnostics, topLevelEnv, rawEnv, field, validate, defaultValue, transformFn)
    : notAllowedInNamedServiceEnvironment(diagnostics, topLevelEnv, rawEnv, field);
}
...
export const isValidName: ValidatorFn = (diagnostics, field, value) => {
  if ((typeof value === 'string' && /^$|^[a-z0-9_ ][a-z0-9-_ ]*$/.test(value)) || value === undefined) {
    return true;
  } else {
    diagnostics.errors.push(
...
export const isObjectWith =
  ...
  (diagnostics, field, value) => {
    if (
      value !== undefined &&
      (typeof value !== 'object' || value === null || !properties.every(prop => prop in value))
    ) {
      diagnostics.errors.push(
    ...
    if (value !== undefined) {
      const restFields = Object.keys(value).filter(key => !properties.includes(key));
...
export const isOneOf =
  ...
  (diagnostics, field, value) => {
    if (value !== undefined && !choices.some(choice => value === choice)) {
      diagnostics.errors.push(
        `Expected "${field}" field to be one of ${JSON.stringify(choices)} but got ${JSON.stringify(value)}.`
      );
      return false;
    }
    return true;
...
export const validateRequiredProperty = (
  ...
  if (value === undefined) {
    ...
    if (!isOneOf(...choices)(diagnostics, `${container}${key}`, value, undefined)) {
      return false;
    }
  }
  return true;
...
export const validateOptionalProperty = (
  ...
  if (value !== undefined) {
    return validateRequiredProperty(diagnostics, container, key, value, type, choices);
  }
  return true;
};
...
export const validateTypedArray = (
  ...
  if (!Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i++) {
      isValid = validateRequiredProperty(diagnostics, container, `[${i}]`, value[i], type) && isValid;
    }
  }
  return isValid;
};
...
export const isRequiredProperty = <T extends object>(
  ...
  hasProperty<T>(obj, prop) && typeof obj[prop] === type && (choices === undefined || choices.includes(obj[prop]));

/**
 * Test to see if `obj` has the optional property `prop` of type `type`.
 */
export const isOptionalProperty = <T extends object>(obj: object, prop: keyof T, type: TypeofType): obj is T =>
  !hasProperty<T>(obj, prop) || typeof obj[prop] === type;

/**
 * Test to see if `obj` has the property `prop`.
 */
export const hasProperty = <T extends object>(obj: object, property: keyof T): obj is T => property in obj;
...
const isBindingList = (
  ...
  value.bindings.every(binding => isRecord(binding) && 'name' in binding && typeof binding.name === 'string');

const isNamespaceList = (value: unknown): value is { binding: string }[] =>
  Array.isArray(value) &&
  value.every(entry => isRecord(entry) && 'binding' in entry && typeof entry.binding === 'string');
