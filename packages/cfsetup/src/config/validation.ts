...
export function normalizeAndValidateConfig(
  ...
  const diagnostics = new Diagnostics(
    `Processing ${configPath ? path.relative(process.cwd(), configPath) : 'wrangler'} configuration:`
  );
  ...
  const isLegacyEnv = (args as { 'legacy-env': boolean | undefined })['legacy-env'] ?? rawConfig.legacy_env ?? true;
  ...
  if (envName !== undefined) {
    ...
    if (rawEnv !== undefined) {
      ...
      const envNames = rawConfig.env
        ? `The available configured environment names are: ${JSON.stringify(Object.keys(rawConfig.env))}\n`
        : '';
      const message =
  ...
  const config: Config = {
    ...
    migrations: normalizeAndValidateMigrations(diagnostics, rawConfig.migrations ?? [], activeEnv.durable_objects),
  ...
  validateAdditionalProperties(diagnostics, 'top-level', Object.keys(rawConfig), [...Object.keys(config), 'env']);
  ...
  if (rawEnv.main !== undefined && rawBuild.upload?.main) {
    ...
    deprecated(
      ...
      `Delete the \`build.upload.main\` and \`build.upload.dir\` fields.\n` +
        ...
        `\`\`\`\n` +
        `main = "${path.join(rawBuild.upload?.dir ?? './dist', rawBuild.upload?.main ?? '.')}"\n` +
        `\`\`\``,
      true
    );
    ...
    watch_dir:
      ...
      command && configPath
        ? Array.isArray(watch_dir)
          ? watch_dir.map(dir => path.relative(process.cwd(), path.join(path.dirname(configPath), `${dir}`)))
          : path.relative(process.cwd(), path.join(path.dirname(configPath), `${watch_dir}`))
        : watch_dir,
    cwd,
    deprecatedUpload
  };
 ...
 */
function normalizeAndValidateDev(diagnostics: Diagnostics, rawDev: RawDevConfig): DevConfig {
  const {
...
function normalizeAndValidateMigrations(
  ...
  if (!Array.isArray(rawMigrations)) {
    diagnostics.errors.push(
      `The optional "migrations" field should be an array, but got ${JSON.stringify(rawMigrations)}`
    );
    return [];
  } else {
    for (let i = 0; i < rawMigrations.length; i++) {
      ...
      validateRequiredProperty(diagnostics, `migrations[${i}]`, `tag`, tag, 'string');
      validateOptionalTypedArray(diagnostics, `migrations[${i}].new_classes`, new_classes, 'string');
      ...
      validateOptionalTypedArray(diagnostics, `migrations[${i}].deleted_classes`, deleted_classes, 'string');
    }
    ...
    if (Array.isArray(durableObjects?.bindings) && durableObjects.bindings.length > 0) {
      ...
      if (exportedDurableObjects.length > 0 && rawMigrations.length === 0) {
        if (
          !exportedDurableObjects.some(exportedDurableObject => typeof exportedDurableObject.class_name !== 'string')
        ) {
...
function normalizeAndValidateSite(
  ...
  if (rawConfig?.site !== undefined) {
    ...
    deprecated(
      ...
      `Delete the \`site.entry-point\` field, then add the top level \`main\` field to your configuration file:\n` +
        ...
        `main = "${path.join(
          ...
          path.extname(String(rawConfig.site['entry-point']) || 'workers-site') ? '' : 'index.js'
        )}"\n` +
        `\`\`\``,
      false,
      undefined,
    ...
    if (configPath && siteEntryPoint) {
      ...
      siteEntryPoint = path.relative(process.cwd(), path.join(path.dirname(configPath), siteEntryPoint));
    }
...
function normalizeAndValidateAssets(
  ...
  const { bucket, include = [], exclude = [], browser_TTL, serve_single_page_app, ...rest } = rawConfig.assets;
  ...
  validateOptionalProperty(diagnostics, 'assets', 'browser_TTL', browser_TTL, 'number');

  validateOptionalProperty(diagnostics, 'assets', 'serve_single_page_app', serve_single_page_app, 'boolean');
    ...
    if (isString(diagnostics, `${field}['${name}']`, filePath, undefined)) {
      if (configPath) {
        mapping[name] = configPath
          ? path.relative(process.cwd(), path.join(path.dirname(configPath), filePath))
          : filePath;
      }
    }
  }
 ...
 */
function mutateEmptyStringAccountIDValue(diagnostics: Diagnostics, rawEnv: RawEnvironment) {
 ...
 */
function mutateEmptyStringRouteValue(diagnostics: Diagnostics, rawEnv: RawEnvironment): RawEnvironment {
  if (rawEnv['route'] === '') {
    diagnostics.warnings.push(
...
function normalizeAndValidatePlacement(
  ...
  if (rawEnv.placement) {
    validateRequiredProperty(diagnostics, 'placement', 'mode', rawEnv.placement.mode, 'string', ['off', 'smart']);
  }

  return inheritable(diagnostics, topLevelEnv, rawEnv, 'placement', () => true, undefined);
}

function validateTailConsumer(diagnostics: Diagnostics, field: string, value: TailConsumer) {
  ...
  isValid = isValid && validateRequiredProperty(diagnostics, field, 'service', value.service, 'string');
  isValid = isValid && validateOptionalProperty(diagnostics, field, 'environment', value.environment, 'string');
...
function normalizeAndValidateEnvironment(
  ...
  const workers_dev = inheritable(diagnostics, topLevelEnv, rawEnv, 'workers_dev', isBoolean, undefined);
  ...
  const environment: Environment = {
    // Inherited fields
    account_id,
    compatibility_date: inheritable(diagnostics, topLevelEnv, rawEnv, 'compatibility_date', isString, undefined),
    ...
    base_dir: normalizeAndValidateBaseDirField(
      configPath,
      inheritable(diagnostics, topLevelEnv, rawEnv, 'base_dir', isString, undefined)
    ),
    route,
    routes,
    triggers: inheritable(diagnostics, topLevelEnv, rawEnv, 'triggers', isObjectWith('crons'), { crons: [] }),
    usage_model: inheritable(diagnostics, topLevelEnv, rawEnv, 'usage_model', isOneOf('bundled', 'unbound'), undefined),
    ...
    logfwdr: inheritable(diagnostics, topLevelEnv, rawEnv, 'logfwdr', validateCflogfwdrObject(envName), {
      bindings: []
    }),
    no_bundle: inheritable(diagnostics, topLevelEnv, rawEnv, 'no_bundle', isBoolean, undefined),
    minify: inheritable(diagnostics, topLevelEnv, rawEnv, 'minify', isBoolean, undefined),
    node_compat: inheritable(diagnostics, topLevelEnv, rawEnv, 'node_compat', isBoolean, undefined),
    first_party_worker: inheritable(diagnostics, topLevelEnv, rawEnv, 'first_party_worker', isBoolean, undefined),
    logpush: inheritable(diagnostics, topLevelEnv, rawEnv, 'logpush', isBoolean, undefined)
  };
...
function validateAndNormalizeTsconfig(
  ...
  const tsconfig = inheritable(diagnostics, topLevelEnv, rawEnv, 'tsconfig', isString, undefined);

  return configPath && tsconfig
    ? path.relative(process.cwd(), path.join(path.dirname(configPath), tsconfig))
    : tsconfig;
}
...
const validateRules =
  ...
  (diagnostics, field, envValue, config) => {
    ...
    const fieldPath = config === undefined ? `${field}` : `env.${envName}.${field}`;
...
const validateRule: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (!isRequiredProperty(rule, 'type', 'string', ['ESModule', 'CommonJS', 'CompiledWasm', 'Text', 'Data'])) {
    diagnostics.errors.push(
  ...
  (diagnostics, field, value, config) => {
    ...
    if (value !== undefined) {
      ...
      if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        ...
        diagnostics.errors.push(
          `The field "${fieldPath}.bindings" should be an array but got ${JSON.stringify(value.bindings)}.`
        );
        isValid = false;
      } else {
        for (let i = 0; i < value.bindings.length; i++) {
          ...
          isValid = validateBinding(bindingDiagnostics, `${fieldPath}.bindings[${i}]`, binding, config) && isValid;
          diagnostics.addChild(bindingDiagnostics);
        }
      }

      const configBindingNames = getBindingNames(config?.[field as keyof Environment]);
...
const validateUnsafeSettings =
  ...
  (diagnostics, field, value, config) => {
    ...
    if (hasProperty(value, 'bindings') && value.bindings !== undefined) {
      const validateBindingsFn = validateBindingsProperty(envName, validateUnsafeBinding);
    ...
    if (
      ...
      diagnostics.errors.push(
        `The field "${fieldPath}.metadata" should be an object but got ${JSON.stringify(value.metadata)}.`
      );
      return false;
    }
    ...
    if (hasProperty(value, 'capnp') && value.capnp !== undefined) {
      if (typeof value.capnp !== 'object' || value.capnp === null || Array.isArray(value.capnp)) {
        diagnostics.errors.push(
          `The field "${fieldPath}.capnp" should be an object but got ${JSON.stringify(value.capnp)}.`
        );
        return false;
      }
      ...
      if (hasProperty(value.capnp, 'compiled_schema')) {
        ...
        if (
          !validateTypedArray(diagnostics, `${fieldPath}.capnp.source_schemas`, value.capnp.source_schemas, 'string')
        ) {
          return false;
        }
      }
 ...
 */
const validateDurableObjectBinding: ValidatorFn = (diagnostics, field, value) => {
  if (typeof value !== 'object' || value === null) {
...
const validateCflogfwdrObject: (env: string) => ValidatorFn = envName => (diagnostics, field, value, topLevelEnv) => {
  //validate the bindings property first, as this also validates that it's an object, etc.
  const bindingsValidation = validateBindingsProperty(envName, validateCflogfwdrBinding);
  if (!bindingsValidation(diagnostics, field, value, topLevelEnv)) return false;
...
const validateBindingArray =
  ...
  (diagnostics, field, envValue, config) => {
    ...
    let isValid = true;
    for (let i = 0; i < envValue.length; i++) {
      isValid = validateBinding(diagnostics, `${fieldPath}[${i}]`, envValue[i], config) && isValid;
    }
...
const validateKVBinding: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (!isOptionalProperty(value, 'preview_id', 'string')) {
    diagnostics.errors.push(
      `"${field}" bindings should, optionally, have a string "preview_id" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  return isValid;
...
const validateQueueBinding: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (!validateAdditionalProperties(diagnostics, field, Object.keys(value), ['binding', 'queue'])) {
    return false;
  }
  ...
  if (!isRequiredProperty(value, 'queue', 'string') || (value as { queue: string }).queue.length === 0) {
...
const validateR2Binding: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (
    ...
    diagnostics.errors.push(
      `"${field}" bindings should have a string "bucket_name" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  ...
  if (!isOptionalProperty(value, 'jurisdiction', 'string')) {
    diagnostics.errors.push(
      `"${field}" bindings should, optionally, have a string "jurisdiction" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  return isValid;
...
const validateConstellationBinding: ValidatorFn = (diagnostics, field, value) => {
  if (typeof value !== 'object' || value === null) {
    diagnostics.errors.push(`"constellation" bindings should be objects, but got ${JSON.stringify(value)}`);
    return false;
  }
  let isValid = true;
  ...
  if (!isRequiredProperty(value, 'project_id', 'string')) {
    diagnostics.errors.push(`"${field}" bindings must have a "project_id" field but got ${JSON.stringify(value)}.`);
    isValid = false;
  }
...
const validateBindingsHaveUniqueNames = (
  ...
  for (const bindingName in bindingsGroupedByName) {
    ...
    const sameType = bindingTypes
      ...
      .filter((type, i, duplicateBindingTypes) => duplicateBindingTypes.indexOf(type) === i);
...
const validateServiceBinding: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (!isRequiredProperty(value, 'service', 'string')) {
    diagnostics.errors.push(
      `"${field}" bindings should have a string "service" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  if (!isOptionalProperty(value, 'environment', 'string')) {
    diagnostics.errors.push(
      `"${field}" bindings should have a string "environment" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  return isValid;
...
const validateAnalyticsEngineBinding: ValidatorFn = (diagnostics, field, value) => {
  ...
  if (!isOptionalProperty(value, 'dataset', 'string') || (value as { dataset: string }).dataset?.length === 0) {
    diagnostics.errors.push(
      `"${field}" bindings should, optionally, have a string "dataset" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  return isValid;
...
const validateWorkerNamespaceBinding: ValidatorFn = (diagnostics, field, value) => {
  if (typeof value !== 'object' || value === null) {
...
function validateWorkerNamespaceOutbound(
  ...
  isValid = isValid && validateRequiredProperty(diagnostics, field, 'service', value.service, 'string');
  isValid = isValid && validateOptionalProperty(diagnostics, field, 'environment', value.environment, 'string');
  isValid = isValid && validateOptionalTypedArray(diagnostics, `${field}.parameters`, value.parameters, 'string');

  return isValid;
}

const validateMTlsCertificateBinding: ValidatorFn = (diagnostics, field, value) => {
  if (typeof value !== 'object' || value === null) {
  ...
  if (
    ...
    diagnostics.errors.push(
      `"${field}" bindings should have a string "certificate_id" field but got ${JSON.stringify(value)}.`
    );
    isValid = false;
  }
  return isValid;
...
function validateQueues(envName: string): ValidatorFn {
  return (diagnostics, field, value, config) => {
    ...
    let isValid = true;
    if (!validateAdditionalProperties(diagnostics, fieldPath, Object.keys(value), ['consumers', 'producers'])) {
      isValid = false;
    }
    ...
    if (hasProperty(value, 'consumers')) {
      ...
      if (!Array.isArray(consumers)) {
        diagnostics.errors.push(
          `The field "${fieldPath}.consumers" should be an array but got ${JSON.stringify(consumers)}.`
        );
        isValid = false;
      }
    ...
    if (hasProperty(value, 'producers')) {
      if (
        !validateBindingArray(envName, validateQueueBinding)(diagnostics, `${field}.producers`, value.producers, config)
      ) {
        isValid = false;
      }
    }
...
const validateConsumer: ValidatorFn = (diagnostics, field, value, _config) => {
  ...
  if (!isRequiredProperty(value, 'queue', 'string')) {
    diagnostics.errors.push(`"${field}" should have a string "queue" field but got ${JSON.stringify(value)}.`);
  }
  ...
  for (const optionalOpt of options) {
    if (!isOptionalProperty(value, optionalOpt.key, optionalOpt.type)) {
      diagnostics.errors.push(
        `"${field}" should, optionally, have a ${optionalOpt.type} "${optionalOpt.key}" field but got ${JSON.stringify(
          value
        )}.`
      );
      isValid = false;
...
function normalizeAndValidateLimits(
  ...
  if (rawEnv.limits) {
    validateRequiredProperty(diagnostics, 'limits', 'cpu_ms', rawEnv.limits.cpu_ms, 'number');
  }

  return inheritable(diagnostics, topLevelEnv, rawEnv, 'limits', () => true, undefined);
}
