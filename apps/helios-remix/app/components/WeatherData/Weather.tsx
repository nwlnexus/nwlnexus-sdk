...
const Weather = ({ apiKey, size = 'lg', className, data, ...props }: WeatherProps): ReactNode => {
  const wData = {
    w: data,
          ...
          <div className="grid grid-rows-2 items-center justify-end">
            ...
            <div className="row-span-1 flex flex-col items-end text-xs font-thin">
              <div className="flex flex-row space-x-2">
                ...
                <span>
                  {Math.round(wData.w.forecast.forecastday[0].day.maxtemp_f)}&deg;F/
                  {Math.round(wData.w.forecast.forecastday[0].day.mintemp_f)}&deg;F
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 items-center justify-end">
          ...
          <div className="col-span-1 flex flex-col items-end">
            ...
            <span className="items-center text-xs font-semibold lg:text-sm">
              H: {Math.round(wData.w.forecast.forecastday[0].day.maxtemp_f)}
              &deg;F L: {Math.round(wData.w.forecast.forecastday[0].day.mintemp_f)}
              &deg;F
            </span>
