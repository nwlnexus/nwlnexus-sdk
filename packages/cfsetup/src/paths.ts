...
export function toUrlPath(filePath: string): UrlPath {
  assert(!/^[a-z]:/i.test(filePath), 'Tried to convert a Windows file path with a drive to a URL path.');
 ...
 */
export function getWranglerTmpDir(projectRoot: string | undefined, prefix: string): EphemeralDirectory {
