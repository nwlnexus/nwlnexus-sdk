...
let debugSpy: MockInstance, logSpy: MockInstance, infoSpy: MockInstance, errorSpy: MockInstance, warnSpy: MockInstance;
...
function normalizeOutput(spy: MockInstance): string {
  return normalizeErrorMarkers(
    replaceByte(stripTrailingWhitespace(normalizeSlashes(normalizeTempDirs(stripTimings(captureCalls(spy))))))
  );
}

function captureCalls(spy: MockInstance): string {
  return spy.mock.calls.map((args: unknown[]) => util.format('%s', ...args)).join('\n');
}
